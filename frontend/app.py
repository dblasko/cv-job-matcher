...
def main():
    st.set_page_config(page_title="CV-based Job Matching Tool", layout="centered" if not LAYOUT_WIDE else "wide")
    ...
    with col1:
        st.write("Upload your CV and find the best job matches! <br/>**For ethical reasons, information about your age, gender or name will be ignored.**", unsafe_allow_html=True)
        if LAYOUT_WIDE:
    ...
    with col2:
        ...
        if not LAYOUT_WIDE:       
            cv_parsed_holder = st.empty()
        match_threshold = st.slider("Minimum match percentage to consider", min_value=0, max_value=100, value=50, step=5)
    ...
    if uploaded_file is not None:
        ...
        job_offers_filtered = [offer for offer in job_offers if offer["match_percentage"] >= match_threshold]
        if not job_offers_filtered:
            st.warning("No job offers matched, verify the file you uploaded or adjust the match percentage threshold!")
        else:
